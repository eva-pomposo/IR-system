...
def add_more_options_to_searcher(searcher_parser):
    
    searcher_parser.add_argument('--mwb', 
                                action="store_true",
                                help='Minimum window boost. If this parameter is passed, the MWB is enabled.')

    searcher_parser.add_argument('--B', 
                                type=int,
                                default=2,
                                help='Maximum multiplicative boost factor.')
...
def searcher_logic(index_folder,
    ...
    if not reader.questions_file:
        ranker.interactive_search(index, reader, tokenizer, top_k=top_k, mwb=mwb, B=B)
    else:
        ranker.batch_search(index, reader, tokenizer, output_file, top_k=top_k, mwb=mwb, B=B)
