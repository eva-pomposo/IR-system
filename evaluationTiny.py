...
for ranking in ["bm25", "tfidf_lnc_ltc", "tfidf_ltc_ltc"]:
    for top_k in [10, 50, 100]:
        for mwb in ["", "--mwb"]:
            ...
            os.system("python3 main.py searcher --reader.class QuestionsWithGSReader results/pubmedSPIMIindexTiny" + ranking + "/mergedFiles questions_with_gs/question_E8B1_gs.jsonl paginator_" + ranking + "_top" + str(top_k) + mwb + ".txt --top_k " + str(top_k) + " " + mwb + " ranking." + ranking.split("_")[0])
            query_throughput = time.time() - query_throughput
            
            paginator = open("paginator_" + ranking + "_top" + str(top_k) + mwb + ".txt", "r")
            ...
            print()
            if mwb == "":
                print("Evaluation metrics for collection Tiny, " + ranking + " , top-" + str(top_k) + " without mwb:")
            else:
                print("Evaluation metrics for collection Tiny, " + ranking + " , top-" + str(top_k) + " with mwb:")
            print("Precision: " + str(precision))
            print("Recall: " + str(recall))
            print("F-measure: " + str(f_measure))
            print("Average Precision: " + str(ap))
            print("Query Throughput: " + str(query_throughput))
            print(paginator.readline())
            print()
            paginator.close()